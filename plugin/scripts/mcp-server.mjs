...
// 工具0：依赖检查
server.tool("video-check-deps", "检查 video-learn 所需的系统依赖是否已安装（ffmpeg、yt-dlp、Python 3.10+、faster-whisper 等）", {}, async () => {
    const deps = [];
    const missing = [];
...
// 工具1：视频下载
server.tool("video-download", "下载或获取视频文件。支持本地路径、YouTube URL、B站 URL", {
...
// 工具2：视频截帧
server.tool("video-extract-frames", "从视频中提取关键帧图片。支持固定间隔和智能场景检测两种模式", {
    ...
    try {
        ...
        if (interval_seconds)
            config.extractor.interval_seconds = interval_seconds;
...
// 工具3：语音转文字
server.tool("video-transcribe", "将视频中的语音转录为带时间戳的文字。使用 faster-whisper 本地模型", {
    video_path: z.string().describe("视频文件路径"),
    output_dir: z.string().describe("输出目录"),
...
// 工具4：图文配对
server.tool("video-assemble", "将截帧图片与转录文字按时间戳配对，生成 paired_results.json", {
