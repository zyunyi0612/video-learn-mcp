...
export async function assembleResults(frameFiles, frameTimestamps, segments, intervalSeconds, outputDir) {
    const items = frameFiles.map((framePath, i) => {
        const ts = frameTimestamps[i];
        const windowStart = ts;
        const windowEnd = ts + intervalSeconds;
        const matched = segments.filter((seg) => seg.end > windowStart && seg.start < windowEnd);
