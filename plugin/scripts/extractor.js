...
export async function extractFrames(videoPath, outputDir, config) {
...
function buildFfmpegArgs(videoPath, outputPattern, config) {
    ...
    if (config.mode === "smart") {
        args.push("-vf", `select='gt(scene\\,${config.scene_threshold})'`, "-vsync", "vfr");
    }
    else {
        args.push("-vf", `fps=1/${config.interval_seconds}`);
    }
    args.push("-q:v", String(Math.max(1, Math.min(31, Math.round((100 - config.image_quality) * 31 / 100)))), outputPattern);
    return args;
}
