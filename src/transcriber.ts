...
import { WhisperConfig } from "./config.js";
 ...
 */
async function readYoutubeSubs(videoPath: string): Promise<TranscribeSegment[] | null> {
 ...
 * 解析 VTT 字幕格式
 */
function parseVtt(content: string): TranscribeSegment[] {
 ...
 */
function parseYouTubeJson(content: string): TranscribeSegment[] {
  const segments: TranscribeSegment[] = [];
  try {
 ...
 * 解析 SRT 字幕格式
 */
function parseSrt(content: string): TranscribeSegment[] {
  const segments: TranscribeSegment[] = [];
...
export async function transcribeVideo(
  ...
  if (youtubeSubs && youtubeSubs.length > 0) {
    ...
    try {
      ...
      const whisperSegments: TranscribeSegment[] = (whisperResult.segments || []).map((s: any) => ({
        ...s,
        source: "whisper"
      }));
    ...
    segments = (result.segments || []).map((s: any) => ({ ...s, source: "whisper" as const }));
 ...
 */
function mergeSubs(youtubeSegs: TranscribeSegment[], whisperSegs: TranscribeSegment[]): TranscribeSegment[] {
  ...
  for (const ws of whisperSegs) {
    ...
    const isCovered = youtubeCovered.some(yc =>
      ws.start >= yc.start - 0.5 && ws.end <= yc.end + 0.5
    );
